...
- **Le certificat est un engagement** : en changer plus tard fait perdre la permission Accessibilité à tous les utilisateurs installés (re-octroi manuel général — et pas un simple toggle : il faut supprimer puis ré-ajouter l'app dans Réglages, piège n° 3 du spike). Sa stabilité est validée par un **spike bloquant** avant toute release publique (permission conservée entre deux builds signés du même certificat ?). **Verdict (spike #87, 2026-07-21) : VALIDÉ** — la permission survit au remplacement de `~/Applications/island.app` entre deux builds signés du certificat `island-release`, et se perd entre deux builds ad-hoc (contraste prouvé). Protocole, preuves et contrat secrets (`ISLAND_CERT_P12`, `ISLAND_CERT_P12_PASSWORD`) : `docs/spikes/87-certificat-stable-accessibilite.md`.
