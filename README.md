...
**Vendoring**: [DynamicNotchKit](https://github.com/MrKai77/DynamicNotchKit) 1.1.0 (MIT, ADR-0003) is vendored in `Vendor/` with a micro-patch: SwiftUI macros (`@Entry`, `#Preview`) don't compile with the Command Line Tools alone (macro plugins require Xcode). The patch replaces `@Entry` with explicit `EnvironmentKey`s and removes the `#Preview`s; behavior identical to upstream. Further micro-patches (first-mouse clicks #33, full-screen Spaces #103, `hide()` continuation leak #131) are marked `island patch` in the source and listed in ADR-0003; their guards run via `swift test --package-path Vendor/DynamicNotchKit` (the root gate does not build a path dependency's test target). With a full Xcode installed, the URL dependency in `Package.swift` can be restored after reconciling those patches.
